...
class IssueBuilder:
    ...
    def build_title(self, spec_version: str, old_version: str) -> str:
        ...
        Returns:
            Issue title
        """
        return f"API Schema Update Required: Microsoft Graph OpenAPI {old_version} → {spec_version}"
    ...
    def build_body(
        ...
        if breaking_count > 0:
            body_parts.extend([
                f"⚠️ **{breaking_count} schema(s) have breaking changes** (removed properties, type changes, or new required fields).",
                "",
            ])
        ...
        body_parts.extend([
            ...
            "",
            "---",
            f"🤖 Auto-generated by OpenAPI schema detection on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        ])
