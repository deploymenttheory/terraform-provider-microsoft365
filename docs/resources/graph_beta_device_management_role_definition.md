...
resource "microsoft365_graph_beta_device_management_role_definition" "example" {
  display_name       = "Device Management Role Definition"
  description        = "Custom role for device management with scoped permissions"
...
- `allowed_resource_actions` (Set of String) Allowed actions for this role permission. This field is equivalent to 'actions' and can be used interchangeably. The API will consolidate values from both fields. Each action must start with 'Microsoft.Intune_'.
