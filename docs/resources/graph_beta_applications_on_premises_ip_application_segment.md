...
- `destination_host` (String) Either the IP address, IP range, or FQDN of the application segment, with or without wildcards. For `destination_type = "ipRange"`, use the start and end addresses separated by `..`, for example `192.168.1.1..192.168.1.10`. For CIDR notation such as `192.168.1.0/24`, use `destination_type = "ipRangeCidr"`; the Graph API normalizes CIDR hosts to `ipRangeCidr` regardless of the requested type.
