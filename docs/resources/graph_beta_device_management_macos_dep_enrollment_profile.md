...
- `enabled_skip_keys` (Set of String) Set of Setup Assistant skip keys (Apple `SkipKeys`) to enable for this profile. macOS-applicable values include: `Appearance`, `Accessibility`, `Biometric`, `Diagnostics`, `DisplayTone`, `FileVault`, `iCloudDiagnostics`, `iCloudStorage`, `Location`, `Payment`, `Privacy`, `Restore`, `ScreenTime`, `Siri`, `TOS`, `Android` (migration), `Appearance`, `TrueTone`, `UnlockWithWatch`, `Wallpaper`, `Registration`, `Passcode`, `Welcome`, `LockdownMode`, `Intelligence`.
...
- `dep_onboarding_settings_id` (String) Identifier of the parent depOnboardingSetting that contains this macOS DEP enrollment profile. This is resolved from the `/deviceManagement` endpoint and correlated to the intuneAccountId.
