...
data "microsoft365_graph_beta_windows_updates_catalog_enteries" "all_updates" {
  filter_type = "all"
}

output "all_catalog_entries" {
  value = data.microsoft365_graph_beta_windows_updates_catalog_enteries.all_updates.entries
}
...
data "microsoft365_graph_beta_windows_updates_catalog_enteries" "security_updates" {
  filter_type  = "display_name"
  filter_value = "SecurityUpdate"
}

output "security_updates" {
  value = data.microsoft365_graph_beta_windows_updates_catalog_enteries.security_updates.entries
}
...
output "specific_update_details" {
  value = length(data.microsoft365_graph_beta_windows_updates_catalog_enteries.specific_update.entries) > 0 ? data.microsoft365_graph_beta_windows_updates_catalog_enteries.specific_update.entries[0] : null
}

# Example 6: Get quality updates with CVE information
data "microsoft365_graph_beta_windows_updates_catalog_enteries" "quality_updates_with_cves" {
  filter_type  = "catalog_entry_type"
  filter_value = "qualityUpdate"
}
...
output "updates_with_exploited_cves" {
  value = [
    for entry in data.microsoft365_graph_beta_windows_updates_catalog_enteries.quality_updates_with_cves.entries :
