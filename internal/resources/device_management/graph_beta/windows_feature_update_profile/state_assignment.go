...
func MapRemoteAssignmentsToTerraform(ctx context.Context, data *WindowsFeatureUpdateProfileResourceModel, assignments []graphmodels.WindowsFeatureUpdateProfileAssignmentable) {
	...
	for _, assignment := range assignments {
		...
		if groupTarget, ok := target.(graphmodels.GroupAssignmentTargetable); ok && groupTarget != nil && groupTarget.GetGroupId() != nil {
			...
			if odataType := groupTarget.GetOdataType(); odataType != nil {
				...
				tflog.Error(ctx, fmt.Sprintf("Inclusion group ID returned an upsupported odataType: %s", groupID))
			}
		}
	}
