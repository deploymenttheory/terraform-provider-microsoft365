...
func init() {
	...
	mocks.GlobalRegistry.Register("windows_updates_compliance_changes", &ComplianceChangesMock{})
}

type ComplianceChangesMock struct{}
...
func (m *ComplianceChangesMock) RegisterMocks() {
	httpmock.RegisterResponder("GET", `=~^https://graph\.microsoft\.com/beta/admin/windows/updates/updatePolicies/[^/]+/complianceChanges`, func(req *http.Request) (*http.Response, error) {
		...
		responsesPath := filepath.Join(sourceDir, "..", "tests", "responses", "validate_get", "get_compliance_changes_success.json")
...
func (m *ComplianceChangesMock) CleanupMockState() {
}
