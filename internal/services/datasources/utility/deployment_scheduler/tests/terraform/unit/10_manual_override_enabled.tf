# Unit Test: Manual override enabled
data "microsoft365_utility_deployment_scheduler" "test" {
  name                  = "Test - Manual Override Enabled"
...
output "condition_met" {
  ...
  description = "Should be true - manual override forces gate open"
}

output "released_scope_id" {
  value       = data.microsoft365_utility_deployment_scheduler.test.released_scope_id
  description = "Should be the GUID - released despite future deployment_start_time"
}
