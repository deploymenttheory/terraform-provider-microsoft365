...
const (
	DataSourceName = "graph_beta_cloud_pc_cloud_pcs"
	ReadTimeout    = 180
)
...
func (d *CloudPcsDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves Cloud PCs from Microsoft Intune. Using the endpoint '/deviceManagement/virtualEndpoint/cloudPCs'. Supports filtering by all, id, display_name, user_principal_name, status, or product_type.",
		Attributes: map[string]schema.Attribute{
			"filter_type": schema.StringAttribute{
				Required:            true,
				MarkdownDescription: "Type of filter to apply. Valid values are: `all`, `id`, `display_name`, `user_principal_name`, `status`, `product_type`. Use 'all' to retrieve all Cloud PCs, 'id' to retrieve a specific Cloud PC by its unique identifier, or other filters to narrow down results.",
			},
