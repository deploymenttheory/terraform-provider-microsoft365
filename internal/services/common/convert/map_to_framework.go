...
func MapToFrameworkString(data map[string]interface{}, key string) types.String {
...
func MapToFrameworkBool(data map[string]interface{}, key string) types.Bool {
...
func MapToFrameworkInt32(data map[string]interface{}, key string) types.Int32 {
...
func MapToFrameworkInt64(data map[string]interface{}, key string) types.Int64 {
	switch value := data[key].(type) {
	case int64:
...
func MapToFrameworkFloat64(data map[string]interface{}, key string) types.Float64 {
	switch value := data[key].(type) {
	case float64:
...
func MapToFrameworkStringSet(ctx context.Context, data map[string]interface{}, key string) types.Set {
...
// Returns null if the key doesn't exist or the value is not a []interface{} containing strings.
func MapToFrameworkStringList(ctx context.Context, data map[string]interface{}, key string) types.List {
	if rawSlice, ok := data[key].([]interface{}); ok {
		var strings []string
		for _, item := range rawSlice {
			if str, ok := item.(string); ok {
				strings = append(strings, str)
