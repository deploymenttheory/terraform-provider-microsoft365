...
func (a *RetireManagedDeviceAction) ValidateConfig(ctx context.Context, req action.ValidateConfigRequest, resp *action.ValidateConfigResponse) {
	...
	tflog.Debug(ctx, fmt.Sprintf("Validating retirement action for %d device(s)", len(deviceIDs)))
	...
	for _, deviceID := range deviceIDs {
		...
		tflog.Debug(ctx, fmt.Sprintf("Validated device %s (Name: %s) for retirement", deviceID, deviceName))
	}

	if len(unsupportedDevices) > 0 {
		resp.Diagnostics.AddAttributeError(
			path.Root("device_ids"),
			"Unsupported Devices for Retire",
			fmt.Sprintf("Retire is supported on Windows, iOS, iPadOS, macOS, Android, and ChromeOS devices. The following devices are not supported: %s. "+
