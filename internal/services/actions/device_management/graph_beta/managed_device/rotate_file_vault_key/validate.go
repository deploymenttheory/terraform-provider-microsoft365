...
func (a *RotateFileVaultKeyManagedDeviceAction) ValidateConfig(ctx context.Context, req action.ValidateConfigRequest, resp *action.ValidateConfigResponse) {
		...
		if len(duplicates) > 0 {
			resp.Diagnostics.AddAttributeWarning(
				...
				fmt.Sprintf("The following managed device IDs are duplicated in the configuration: %s. "+
					"FileVault key rotation will only be performed once per device, but you should remove duplicates from your configuration.",
			...
			if managedID == comanagedID {
				resp.Diagnostics.AddAttributeWarning(
					...
					fmt.Sprintf("Device ID %s appears in both managed_device_ids and comanaged_device_ids. "+
						"A device should only be in one list. FileVault key rotation will be attempted for both endpoints, "+
	...
	tflog.Debug(ctx, fmt.Sprintf("Validating rotate FileVault key action for %d managed and %d co-managed device(s)",
	...
	if len(unsupportedManagedDevices) > 0 {
		resp.Diagnostics.AddAttributeError(
			...
			fmt.Sprintf("FileVault key rotation is only supported on macOS devices. The following managed devices are not supported: %s. "+
	...
	if len(unsupportedComanagedDevices) > 0 {
		resp.Diagnostics.AddAttributeError(
			...
			fmt.Sprintf("FileVault key rotation is only supported on macOS devices. The following co-managed devices are not supported: %s. "+
				"Please remove non-macOS devices from the configuration.",
