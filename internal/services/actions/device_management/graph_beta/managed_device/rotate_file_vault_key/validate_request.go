...
// FileVaultValidationResult contains the results of device validation
type FileVaultValidationResult struct {
...
func validateRequest(ctx context.Context, client *msgraphbetasdk.GraphServiceClient, managedDeviceIDs []string, comanagedDeviceIDs []string) (*FileVaultValidationResult, error) {
	result := &FileVaultValidationResult{
	...
	for _, deviceID := range managedDeviceIDs {
		...
		if device.GetOperatingSystem() != nil {
			...
			if os != "macos" && os != "mac" {
				...
				tflog.Warn(ctx, "Managed device is not macOS", map[string]any{
					"device_id": deviceID,
	...
	for _, deviceID := range comanagedDeviceIDs {
		...
		if device.GetOperatingSystem() != nil {
			...
			if os != "macos" && os != "mac" {
				...
				tflog.Warn(ctx, "Co-managed device is not macOS", map[string]any{
					"device_id": deviceID,
					"os":        *device.GetOperatingSystem(),
				})
				continue
