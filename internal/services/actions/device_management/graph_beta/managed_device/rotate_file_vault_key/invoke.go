...
func (a *RotateFileVaultKeyManagedDeviceAction) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	...
	tflog.Debug(ctx, "Starting FileVault key rotation action", map[string]any{"action": ActionName})
	...
	tflog.Debug(ctx, "Processing devices for FileVault key rotation", map[string]any{
	...
	if validateDeviceExists {
		...
		results := validation.NewResults().
			...
			Error(validationResult.UnsupportedManagedDevices, "managed device", "are not macOS devices. FileVault key rotation only works on macOS devices").
			Error(validationResult.UnsupportedComanagedDevices, "co-managed device", "are not macOS devices. FileVault key rotation only works on macOS devices")
	...
	progressTracker := progress.For(resp).WithTotalDevices(totalDevices).
		Starting("FileVault key rotation", fmt.Sprintf("%d devices (%d managed, %d co-managed)", totalDevices, len(managedDeviceIDs), len(comanagedDeviceIDs)))

	// Process managed devices sequentially
	for _, deviceID := range managedDeviceIDs {
		err := a.rotateFileVaultKeyManagedDevice(ctx, deviceID)
		if err != nil {
			progressTracker.Device(deviceID, "").Failed(err.Error())
			tflog.Error(ctx, "Failed to rotate FileVault key on managed device", map[string]any{
				"device_id": deviceID,
				"error":     err.Error(),
			})
		} else {
			progressTracker.Device(deviceID, "").Succeeded("FileVault key rotated")
			tflog.Info(ctx, "Successfully rotated FileVault key on managed device", map[string]any{
				"device_id": deviceID,
			})
		}
	}
	...
	for _, deviceID := range comanagedDeviceIDs {
		err := a.rotateFileVaultKeyComanagedDevice(ctx, deviceID)
		if err != nil {
			progressTracker.Device(deviceID, "").Failed(err.Error())
			tflog.Error(ctx, "Failed to rotate FileVault key on co-managed device", map[string]any{
				"device_id": deviceID,
				"error":     err.Error(),
			})
		} else {
			progressTracker.Device(deviceID, "").Succeeded("FileVault key rotated")
			tflog.Info(ctx, "Successfully rotated FileVault key on co-managed device", map[string]any{
				"device_id": deviceID,
			})
		}
	}
	...
	if progressTracker.HasFailures() {
		if ignorePartialFailures {
			progressTracker.CompletedWithIgnoredFailures("FileVault key rotation")
			tflog.Warn(ctx, "FileVault key rotation action completed with ignored failures", map[string]any{
				"success_count": progressTracker.SuccessCount(),
				"failed_count":  progressTracker.FailureCount(),
			})
		} else {
			progressTracker.Failed("FileVault Key Rotation Failed", "rotate FileVault keys on devices")
			return
		}
	} else {
		progressTracker.CompletedSuccessfully("rotated all FileVault recovery keys. New recovery keys have been generated and escrowed with Intune. Previous recovery keys are now invalid")
	}

	tflog.Info(ctx, "FileVault key rotation action completed", map[string]any{
		"success_count":            progressTracker.SuccessCount(),
		"failed_count":             progressTracker.FailureCount(),
		"total_devices":            totalDevices,
...
func (a *RotateFileVaultKeyManagedDeviceAction) rotateFileVaultKeyManagedDevice(ctx context.Context, deviceID string) error {
	tflog.Debug(ctx, "Rotating FileVault key on managed device", map[string]any{
		"device_id": deviceID,
	})
...
func (a *RotateFileVaultKeyManagedDeviceAction) rotateFileVaultKeyComanagedDevice(ctx context.Context, deviceID string) error {
	tflog.Debug(ctx, "Rotating FileVault key on co-managed device", map[string]any{
		"device_id": deviceID,
	})
