...
func (a *RebootNowManagedDeviceAction) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	...
	tflog.Debug(ctx, "Starting reboot now action", map[string]any{"action": ActionName})
	...
	tflog.Debug(ctx, "Processing devices for reboot", map[string]any{
	...
	if validateDeviceExists {
		...
		results := validation.NewResults().
			...
			Error(validationResult.UnsupportedOSDevices, "device", "do not support remote reboot (only Windows, macOS, ChromeOS, and supervised iOS/iPadOS are supported)").
			Warning(validationResult.OfflineDevices, "device", "may be offline or not properly registered (reboot command will be queued until device checks in)")
	...
	progressTracker := progress.For(resp).WithTotalDevices(totalDevices).
		Starting("reboot", fmt.Sprintf("%d devices", totalDevices))

	// Process devices sequentially
	for _, deviceID := range deviceIDs {
		err := a.rebootNow(ctx, deviceID)
		if err != nil {
			progressTracker.Device(deviceID, "").Failed(err.Error())
			tflog.Error(ctx, "Failed to reboot device", map[string]any{
				"device_id": deviceID,
				"error":     err.Error(),
			})
		} else {
			progressTracker.Device(deviceID, "").Succeeded("reboot command sent")
			tflog.Info(ctx, "Successfully sent reboot command to device", map[string]any{
				"device_id": deviceID,
			})
		}
	}
	...
	if progressTracker.HasFailures() {
		if ignorePartialFailures {
			progressTracker.CompletedWithIgnoredFailures("reboot")
			tflog.Warn(ctx, "Reboot action completed with ignored failures", map[string]any{
				"success_count": progressTracker.SuccessCount(),
				"failed_count":  progressTracker.FailureCount(),
			})
		} else {
			progressTracker.Failed("Reboot Action Failed", "reboot devices")
			return
		}
	} else {
		progressTracker.CompletedSuccessfully("sent reboot command to all devices. Online devices will restart immediately. Offline devices will restart when they next check in with Intune")
	}

	tflog.Info(ctx, "Reboot now action completed", map[string]any{
		"success_count":            progressTracker.SuccessCount(),
		"failed_count":             progressTracker.FailureCount(),
		"total_devices":            totalDevices,
...
func (a *RebootNowManagedDeviceAction) rebootNow(ctx context.Context, deviceID string) error {
	tflog.Debug(ctx, "Sending reboot command to device", map[string]any{
		"device_id": deviceID,
	})
