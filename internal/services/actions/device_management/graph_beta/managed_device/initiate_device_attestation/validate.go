...
func (a *InitiateDeviceAttestationManagedDeviceAction) ValidateConfig(ctx context.Context, req action.ValidateConfigRequest, resp *action.ValidateConfigResponse) {
	...
	tflog.Debug(ctx, fmt.Sprintf("Validating device attestation action for %d managed and %d co-managed device(s)",
	...
	for _, deviceID := range managedDeviceIDs {
		...
		if err != nil {
			...
			// Check if device is Windows (device attestation is Windows-specific)
	...
	if len(unsupportedManagedDevices) > 0 {
		resp.Diagnostics.AddAttributeWarning(
			...
			fmt.Sprintf("The following managed devices are not Windows devices: %s. "+
				"Device attestation is only supported on Windows devices with TPM 1.2/2.0 and secure boot enabled. "+
	...
	if len(managedDeviceIDs)+len(comanagedDeviceIDs) > 0 {
		resp.Diagnostics.AddWarning(
			"Device Attestation Operation",
			fmt.Sprintf("This action will initiate device attestation for %d device(s).\n\n"+
				"Important notes:\n"+
				"- Uses TPM to cryptographically verify device health and security state\n"+
				"- Creates attestation report for conditional access and compliance policies\n"+
				...
				"- Devices must have TPM 1.2/2.0 and secure boot enabled",
				len(managedDeviceIDs)+len(comanagedDeviceIDs)),
		)
	}
}
