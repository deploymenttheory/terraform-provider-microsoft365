...
func (a *DisableManagedDeviceAction) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
	...
	tflog.Debug(ctx, "Starting device disable action", map[string]any{"action": ActionName})
	...
	tflog.Debug(ctx, "Processing devices for disable", map[string]any{
	...
	for _, deviceID := range managedDeviceIDs {
		err := a.disableManagedDevice(ctx, deviceID)
		if err != nil {
			progressTracker.Device(deviceID, "Managed").Failed(err.Error())
			tflog.Error(ctx, "Failed to disable managed device", map[string]any{
				"device_id": deviceID,
				"error":     err.Error(),
			})
		} else {
			progressTracker.Device(deviceID, "Managed").Succeeded("disabled successfully")
			tflog.Info(ctx, "Successfully disabled managed device", map[string]any{
				"device_id": deviceID,
			})
		}
	}
	...
	for _, deviceID := range comanagedDeviceIDs {
		err := a.disableComanagedDevice(ctx, deviceID)
		if err != nil {
			progressTracker.Device(deviceID, "Co-managed").Failed(err.Error())
			tflog.Error(ctx, "Failed to disable co-managed device", map[string]any{
				"device_id": deviceID,
				"error":     err.Error(),
			})
		} else {
			progressTracker.Device(deviceID, "Co-managed").Succeeded("disabled successfully")
			tflog.Info(ctx, "Successfully disabled co-managed device", map[string]any{
				"device_id": deviceID,
			})
		}
	}
	...
	if progressTracker.HasFailures() {
		if ignorePartialFailures {
			progressTracker.CompletedWithIgnoredFailures("device disable")
			tflog.Warn(ctx, "Device disable completed with ignored failures", map[string]any{
				"success_count": progressTracker.SuccessCount(),
				"failed_count":  progressTracker.FailureCount(),
			})
		} else {
			progressTracker.Failed("Device Disable Failed", "disable devices")
			return
		}
	} else {
		progressTracker.CompletedSuccessfully("disabled all devices. Devices will not sync with Intune until re-enabled")
	}

	tflog.Info(ctx, "Device disable action completed", map[string]any{
		"success_count":            progressTracker.SuccessCount(),
		"failed_count":             progressTracker.FailureCount(),
		"total_devices":            totalDevices,
