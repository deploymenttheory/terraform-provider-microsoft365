...
func (a *DisableManagedDeviceAction) ValidateConfig(ctx context.Context, req action.ValidateConfigRequest, resp *action.ValidateConfigResponse) {
		...
		if len(duplicates) > 0 {
			resp.Diagnostics.AddAttributeWarning(
				...
				fmt.Sprintf("The following managed device IDs are duplicated in the configuration: %s. "+
					"Disable will only be performed once per device, but you should remove duplicates from your configuration.",
			...
			if managedID == comanagedID {
				resp.Diagnostics.AddAttributeWarning(
					...
					fmt.Sprintf("Device ID %s appears in both managed_device_ids and comanaged_device_ids. "+
						"A device should only be in one list. Disable will be attempted for both endpoints, "+
