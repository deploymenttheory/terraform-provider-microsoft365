...
// Create handles the Create operation for the iOS/iPadOS ADE enrollment policy.
func (r *IOSiPadOSDeviceEnrollmentPolicyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var object IOSiPadOSDeviceEnrollmentPolicyResourceModel
	...
	if !object.IsDefaultPolicyAssignment.IsNull() && !object.IsDefaultPolicyAssignment.IsUnknown() && object.IsDefaultPolicyAssignment.ValueBool() {
		tflog.Debug(ctx, fmt.Sprintf("Setting policy ID %s as default iOS/iPadOS enrollment profile for dep_onboarding_settings_id %s", object.ID.ValueString(), depOnboardingSettingsId))

		if err := r.setDefaultIOSProfileWithRetry(ctx, depOnboardingSettingsId, object.ID.ValueString()); err != nil {
			errors.HandleKiotaGraphError(ctx, err, resp, constants.TfOperationCreate, r.WritePermissions)
			return
		}
	}
...
// Read handles the Read operation for the iOS/iPadOS ADE enrollment policy.
func (r *IOSiPadOSDeviceEnrollmentPolicyResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var object IOSiPadOSDeviceEnrollmentPolicyResourceModel
...
func (r *IOSiPadOSDeviceEnrollmentPolicyResource) listAllPolicySettingsWithPageIterator(ctx context.Context, policyId string) ([]models.DeviceManagementConfigurationSettingable, error) {
...
// Update handles the Update operation for the iOS/iPadOS ADE enrollment policy.
func (r *IOSiPadOSDeviceEnrollmentPolicyResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan IOSiPadOSDeviceEnrollmentPolicyResourceModel
	var state IOSiPadOSDeviceEnrollmentPolicyResourceModel
	...
	if planIsDefault && !stateIsDefault {
		tflog.Debug(ctx, fmt.Sprintf("Setting policy ID %s as default iOS/iPadOS enrollment profile for dep_onboarding_settings_id %s", state.ID.ValueString(), plan.DepOnboardingSettingsId.ValueString()))

		if err := r.setDefaultIOSProfileWithRetry(ctx, plan.DepOnboardingSettingsId.ValueString(), state.ID.ValueString()); err != nil {
			errors.HandleKiotaGraphError(ctx, err, resp, constants.TfOperationUpdate, r.WritePermissions)
			return
		}
		...
		tflog.Debug(ctx, fmt.Sprintf("Policy ID %s is no longer the default iOS/iPadOS enrollment profile for dep_onboarding_settings_id %s; no Graph action required for is_default_policy_assignment false",
...
// Delete handles the Delete operation for the iOS/iPadOS ADE enrollment policy.
func (r *IOSiPadOSDeviceEnrollmentPolicyResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var object IOSiPadOSDeviceEnrollmentPolicyResourceModel
