// Package mocks provides HTTP mock responders for the ios_ipados_device_enrollment_policy unit tests.
//
...
// per settings combination, since the settings catalog tree here has ~40 independent toggles.
package mocks
...
func (m *IOSiPadOSDeviceEnrollmentPolicyMock) RegisterMocks() {
	...
	httpmock.RegisterResponder("GET", `=~^https://graph\.microsoft\.com/beta/deviceManagement/depOnboardingSettings/[0-9a-fA-F-]+`,
		func(req *http.Request) (*http.Response, error) {
			...
			if hasDefault {
				response["defaultIosEnrollmentProfile"] = map[string]any{
