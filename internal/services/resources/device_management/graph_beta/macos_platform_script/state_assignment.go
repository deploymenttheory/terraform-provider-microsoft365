...
// MacOSPlatformScriptAssignmentType returns the object type for MacOSPlatformScriptAssignmentModel
func MacOSPlatformScriptAssignmentType() attr.Type {
...
func MapAssignmentsToTerraform(ctx context.Context, data *MacOSPlatformScriptResourceModel, assignments []graphmodels.DeviceManagementScriptAssignmentable) {
	...
	for i, assignment := range assignments {
		...
		objValue, diags := types.ObjectValue(MacOSPlatformScriptAssignmentType().(types.ObjectType).AttrTypes, assignmentObj)
		if !diags.HasError() {
	...
	if len(assignmentValues) > 0 {
		setVal, diags := types.SetValue(MacOSPlatformScriptAssignmentType(), assignmentValues)
		if diags.HasError() {
