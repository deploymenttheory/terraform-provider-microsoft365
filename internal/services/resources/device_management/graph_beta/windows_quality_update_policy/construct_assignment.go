...
// constructAssignment constructs and returns a WindowsQualityUpdatePoliciesItemAssignPostRequestBody
func constructAssignment(ctx context.Context, data *WindowsQualityUpdatePolicyResourceModel) (devicemanagement.WindowsQualityUpdatePoliciesItemAssignPostRequestBodyable, error) {
	tflog.Debug(ctx, "Starting Windows Quality Update Policy assignment construction")

	requestBody := devicemanagement.NewWindowsQualityUpdatePoliciesItemAssignPostRequestBody()
	scriptAssignments := make([]graphmodels.WindowsQualityUpdatePolicyAssignmentable, 0)
	...
	for idx, assignment := range terraformAssignments {
		...
		graphAssignment := graphmodels.NewWindowsQualityUpdatePolicyAssignment()
