...
func (r RoleAssignmentTestResource) Exists(ctx context.Context, _ any, state *terraform.InstanceState) (*bool, error) {
	...
	url := httpClient.GetBaseURL() + "/deviceManagement/roleAssignments/" + assignmentID
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("unexpected error checking assignment existence: %s (status: %d)", errorInfo.ErrorCode, errorInfo.StatusCode)
	}
