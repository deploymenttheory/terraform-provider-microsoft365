...
// DefinitionLookupService handles the lookup of group policy definitions
type DefinitionLookupService struct {
	client *msgraphbetasdk.GraphServiceClient
}

// NewDefinitionLookupService creates a new definition lookup service
func NewDefinitionLookupService(client *msgraphbetasdk.GraphServiceClient) *DefinitionLookupService {
	return &DefinitionLookupService{
		client: client,
	}
}
...
// LookupDefinitionID finds the definition ID based on display name, class type, and optional category path
func (s *DefinitionLookupService) LookupDefinitionID(ctx context.Context, displayName, classType, categoryPath string) (string, error) {
	...
	if definitions == nil || definitions.GetValue() == nil || len(definitions.GetValue()) == 0 {
		tflog.Debug(ctx, fmt.Sprintf("[LOOKUP] No definitions found with exact filter (count=0), trying client-side filtering"))

		// If no exact match found, try fetching all and filtering client-side
		allDefinitions, err := s.getAllDefinitions(ctx)
		if err != nil {
			return "", err
		}

		return s.findMatchingDefinition(ctx, allDefinitions, displayName, normalizedClassType, categoryPath)
	}
...
func (s *DefinitionLookupService) getAllDefinitions(ctx context.Context) ([]graphmodels.GroupPolicyDefinitionable, error) {
...
func (s *DefinitionLookupService) findMatchingDefinition(ctx context.Context, definitions []graphmodels.GroupPolicyDefinitionable, displayName, classType, categoryPath string) (string, error) {
...
// LookupDefinitionInfo finds the display name, class type, and category path based on definition ID (for reverse lookup)
func (s *DefinitionLookupService) LookupDefinitionInfo(ctx context.Context, definitionID string) (displayName, classType, categoryPath string, err error) {
	tflog.Debug(ctx, fmt.Sprintf("[LOOKUP] Looking up definition info for ID='%s'", definitionID))

	definition, err := s.client.
		DeviceManagement().
		GroupPolicyDefinitions().
		ByGroupPolicyDefinitionId(definitionID).
		Get(ctx, &devicemanagement.GroupPolicyDefinitionsGroupPolicyDefinitionItemRequestBuilderGetRequestConfiguration{
			QueryParameters: &devicemanagement.GroupPolicyDefinitionsGroupPolicyDefinitionItemRequestBuilderGetQueryParameters{
				Select: []string{"id", "displayName", "classType", "categoryPath"},
			},
		})

	if err != nil {
		tflog.Error(ctx, fmt.Sprintf("[LOOKUP] Failed to fetch definition info for ID='%s': %s", definitionID, err.Error()))
		return "", "", "", fmt.Errorf("failed to fetch group policy definition with ID '%s': %w", definitionID, err)
	}

	if definition == nil {
		return "", "", "", fmt.Errorf("group policy definition with ID '%s' not found", definitionID)
	}

	if definition.GetDisplayName() == nil || definition.GetClassType() == nil {
		return "", "", "", fmt.Errorf("group policy definition with ID '%s' has missing displayName or classType", definitionID)
	}
	...
	tflog.Debug(ctx, fmt.Sprintf("[LOOKUP] Found definition info: DisplayName='%s', ClassType='%s', CategoryPath='%s'", displayName, classType, categoryPath))
	return displayName, classType, categoryPath, nil
}
