...
resource "microsoft365_graph_beta_device_management_assignment_filter" "acc_test_assignment_filter_1" {
  ...
  timeouts = {
    create = "10s"   # Reduced for faster test execution
    read   = "10s"   # Reduced for faster test execution
    update = "10s"   # Reduced for faster test execution
    delete = "20s"  # Longer for cleanup after assignments are removed
  }
}