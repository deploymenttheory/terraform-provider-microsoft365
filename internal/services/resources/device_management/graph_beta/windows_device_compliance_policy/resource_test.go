...
// Test 06: grace_period_hours accepts the full Graph-documented range of 0 to 8760 hours (365 days)
func TestUnitResourceWindowsDeviceCompliancePolicy_06_GracePeriodHoursBounds(t *testing.T) {
	mocks.SetupUnitTestEnvironment(t)
	_, policyMock := setupMockEnvironment()
	defer httpmock.DeactivateAndReset()
	defer policyMock.CleanupMockState()
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: loadUnitTestTerraform("resource_windows_grace_period_bounds.tf"),
				Check: resource.ComposeTestCheckFunc(
					check.That(resourceType+".grace_period_bounds").Key("scheduled_actions_for_rule.0.scheduled_action_configurations.0.grace_period_hours").HasValue("0"),
					check.That(resourceType+".grace_period_bounds").Key("scheduled_actions_for_rule.0.scheduled_action_configurations.1.grace_period_hours").HasValue("8760"),
				),
			},
		},
	})
...
// Test 07: grace_period_hours above the Graph maximum of 8760 is rejected at plan time
func TestUnitResourceWindowsDeviceCompliancePolicy_07_GracePeriodHoursExceedsMaximum(t *testing.T) {
	mocks.SetupUnitTestEnvironment(t)
	_, policyMock := setupMockEnvironment()
	defer httpmock.DeactivateAndReset()
	defer policyMock.CleanupMockState()
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      loadUnitTestTerraform("resource_windows_grace_period_invalid.tf"),
				ExpectError: regexp.MustCompile(`must be between 0 and 8760`),
			},
		},
	})
}
