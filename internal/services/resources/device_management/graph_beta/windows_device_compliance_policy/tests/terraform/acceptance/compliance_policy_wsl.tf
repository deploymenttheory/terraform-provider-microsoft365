...
resource "microsoft365_graph_beta_device_management_device_compliance_notification_template" "acc_test_device_compliance_notification_template" {
  display_name     = "acc-test-dcnt-wsl-${random_string.suffix.result}"
...
resource "microsoft365_graph_beta_device_management_windows_device_compliance_policy" "wsl" {
  display_name       = "acc-test-wdcp-wsl-${random_string.suffix.result}"
  
  depends_on = [time_sleep.wait_for_dependencies]
  description        = "acc-test-wdcp-wsl-${random_string.suffix.result}"
  role_scope_tag_ids = ["0"]
