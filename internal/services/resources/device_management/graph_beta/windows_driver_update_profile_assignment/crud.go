...
// Create handles the Create operation for Windows Driver Update Profile Assignment resources.
//
...
func (r *WindowsDriverUpdateProfileAssignmentResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, fmt.Sprintf("Reading %s with ID: %s for Windows Driver Update Profile: %s",
		ResourceName, object.ID.ValueString(), object.WindowsDriverUpdateProfileId.ValueString()))
