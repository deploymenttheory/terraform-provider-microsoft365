...
import (
	...
	membersMocks "github.com/deploymenttheory/terraform-provider-microsoft365/internal/services/resources/device_management/graph_beta/windows_autopatch_deployment_audience_members/mocks"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("id").MatchesRegex(regexp.MustCompile(`^[0-9a-fA-F-]+_azureADDevice$`)),
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("audience_id").HasValue("test-audience-id-001"),
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("member_type").HasValue("azureADDevice"),
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("members.#").HasValue("2"),
				),
			},
			{
				ResourceName:      WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName + ".test",
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("member_type").HasValue("azureADDevice"),
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("members.#").HasValue("2"),
					check.That(WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName+".test").Key("exclusions.#").HasValue("1"),
				),
			},
			{
				ResourceName:      WindowsAutopatchDeploymentResourceAudienceMembers.ResourceName + ".test",
