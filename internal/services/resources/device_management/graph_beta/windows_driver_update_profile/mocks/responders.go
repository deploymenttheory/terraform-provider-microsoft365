...
var mockState struct {
	sync.Mutex
	driverProfiles map[string]map[string]interface{}
}

func init() {
	mockState.driverProfiles = make(map[string]map[string]interface{})
...
func (m *WindowsDriverUpdateProfileMock) RegisterMocks() {
	...
	httpmock.RegisterResponder("GET", "https://graph.microsoft.com/beta/deviceManagement/windowsDriverUpdateProfiles", func(req *http.Request) (*http.Response, error) {
		...
		list := make([]map[string]interface{}, 0, len(mockState.driverProfiles))
	...
	httpmock.RegisterResponder("DELETE", `=~^https://graph\.microsoft\.com/beta/deviceManagement/windowsDriverUpdateProfiles/[^/]+$`, func(req *http.Request) (*http.Response, error) {
		parts := strings.Split(req.URL.Path, "/")
		id := parts[len(parts)-1]
		mockState.Lock()
