# Acceptance test: Windows Autopilot - User Driven with Group Assignments
# Full dependency chain: random_string -> groups -> time_sleep -> autopilot_profile

resource "random_string" "test_id_03" {
  length  = 8
  special = false
  upper   = false
}
...
resource "microsoft365_graph_beta_groups_group" "test_group_1" {
  display_name     = "acc-test-autopilot-03-group1-${random_string.test_id_03.result}"
  description      = "Test group for autopilot deployment profile acceptance test"
  mail_nickname    = "acc-autopilot-03-g1-${random_string.test_id_03.result}"
  mail_enabled     = false
...
resource "microsoft365_graph_beta_groups_group" "test_group_2" {
  display_name     = "acc-test-autopilot-03-group2-${random_string.test_id_03.result}"
  description      = "Test group for autopilot deployment profile acceptance test"
  mail_nickname    = "acc-autopilot-03-g2-${random_string.test_id_03.result}"
  mail_enabled     = false
  security_enabled = true
  hard_delete      = true
}
...
resource "microsoft365_graph_beta_groups_group" "test_group_3" {
  display_name     = "acc-test-autopilot-03-group3-${random_string.test_id_03.result}"
  description      = "Test group for autopilot deployment profile acceptance test"
  mail_nickname    = "acc-autopilot-03-g3-${random_string.test_id_03.result}"
  mail_enabled     = false
  security_enabled = true
  hard_delete      = true
}
...
resource "microsoft365_graph_beta_device_management_windows_autopilot_deployment_profile" "user_driven" {
  display_name                                 = "acc test user driven autopilot with group assignments"
