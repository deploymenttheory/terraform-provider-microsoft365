...
// WindowsPlatformScriptAssignmentType returns the object type for WindowsPlatformScriptAssignmentModel
func WindowsPlatformScriptAssignmentType() attr.Type {
...
func MapAssignmentsToTerraform(ctx context.Context, data *WindowsPlatformScriptResourceModel, assignments []graphmodels.DeviceManagementScriptAssignmentable) {
	...
	for i, assignment := range assignments {
		...
		objValue, diags := types.ObjectValue(WindowsPlatformScriptAssignmentType().(types.ObjectType).AttrTypes, assignmentObj)
		if !diags.HasError() {
	...
	if len(assignmentValues) > 0 {
		setVal, diags := types.SetValue(WindowsPlatformScriptAssignmentType(), assignmentValues)
		if diags.HasError() {
