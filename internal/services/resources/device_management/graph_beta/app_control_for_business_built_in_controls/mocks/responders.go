...
func (m *AppControlForBusinessBuiltInControlsMock) RegisterMocks() {
	...
	httpmock.RegisterResponder("GET", `=~^https://graph\.microsoft\.com/beta/deviceManagement/configurationPolicies/[^/]+$`, func(req *http.Request) (*http.Response, error) {
		...
		// Get the appropriate template based on policy configuration
		...
		if assignmentsExist && assignments != nil {
			if assignmentSlice, ok := assignments.([]any); ok && len(assignmentSlice) > 0 {
				...
				jsonStr, _ := helpers.ParseJSONFile("../tests/responses/validate_get/get_app_control_enforce.json")
				jsonTemplate = jsonStr
			} else {
				jsonStr, _ := helpers.ParseJSONFile("../tests/responses/validate_get/get_app_control_audit.json")
				jsonTemplate = jsonStr
			}
		} else if strings.Contains(fmt.Sprintf("%v", policy["settings"]), "_enable_app_control_1") {
			jsonStr, _ := helpers.ParseJSONFile("../tests/responses/validate_get/get_app_control_enforce.json")
