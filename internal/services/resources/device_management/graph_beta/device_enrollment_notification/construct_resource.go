...
func constructResource(ctx context.Context, data *DeviceEnrollmentNotificationResourceModel, isForCreate bool, currentTemplateGUIDs ...[]string) (graphmodels.DeviceEnrollmentConfigurationable, error) {
	...
	if !data.PlatformType.IsNull() && !data.PlatformType.IsUnknown() {
		...
		case "android":
			...
			requestBody.SetPlatformType(&platformTypeValue)
		case "ios":
			platformTypeValue := graphmodels.IOS_ENROLLMENTRESTRICTIONPLATFORMTYPE
			requestBody.SetPlatformType(&platformTypeValue)
		case "windows":
			platformTypeValue := graphmodels.WINDOWS_ENROLLMENTRESTRICTIONPLATFORMTYPE
			requestBody.SetPlatformType(&platformTypeValue)
		case "mac":
			platformTypeValue := graphmodels.MAC_ENROLLMENTRESTRICTIONPLATFORMTYPE
			requestBody.SetPlatformType(&platformTypeValue)
		case "linux":
			platformTypeValue := graphmodels.LINUX_ENROLLMENTRESTRICTIONPLATFORMTYPE
			requestBody.SetPlatformType(&platformTypeValue)
		default:
