...
// current default visionOS enrollment profile on Graph.
//
// Microsoft Graph exposes no unset/clear action for the default profile - the only way a policy
// stops being the default is another visionOS profile being promoted via setDefaultProfile. So a
...
// the token's live default (GET /deviceManagement/depOnboardingSettings/{id}?$expand=defaultVisionOSEnrollmentProfile)
...
func (r *VisionOSDeviceEnrollmentPolicyResource) validateRequest(ctx context.Context, plan, state *VisionOSDeviceEnrollmentPolicyResourceModel) error {
	...
	tflog.Debug(ctx, fmt.Sprintf("is_default_policy_assignment true->false requested for policy %s; checking whether it is still the current default visionOS enrollment profile for dep_onboarding_settings_id %s",
	...
	if err != nil {
		return fmt.Errorf("could not resolve the current default visionOS enrollment profile for dep_onboarding_settings_id %s to verify that is_default_policy_assignment can be unset: %w",
			depOnboardingSettingsId, err)
	}

	if isCurrentDefault {
		return fmt.Errorf("cannot unset the default policy assignment: this policy (%s) is currently the default visionOS enrollment profile for its DEP token "+
			...
			"default when a different visionOS enrollment policy on the same token is promoted via its own is_default_policy_assignment = true. Promote the "+
