...
// ADE token) that owns visionOS enrollment policies.
//
// Resolution order:
...
func (r *VisionOSDeviceEnrollmentPolicyResource) resolveDepOnboardingSettingsId(ctx context.Context, provided types.String) (string, error) {
	...
	for _, s := range settings.GetValue() {
		...
		// Only Apple ADE/ABM (dep) and ASM tokens own visionOS enrollment policies.
