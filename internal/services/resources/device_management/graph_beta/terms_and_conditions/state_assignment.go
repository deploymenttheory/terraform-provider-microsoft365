...
// TermsAndConditionsAssignmentType returns the object type for TermsAndConditionsAssignmentModel
func TermsAndConditionsAssignmentType() attr.Type {
...
func MapRemoteAssignmentStateToTerraform(ctx context.Context, terraform *TermsAndConditionsResourceModel, assignmentsResponse graphmodels.TermsAndConditionsAssignmentCollectionResponseable) {
	if assignmentsResponse == nil {
		terraform.Assignments = types.SetNull(TermsAndConditionsAssignmentType())
		return
	}
...
// MapAssignmentsToTerraform maps the remote TermsAndConditions assignments to Terraform state
func MapAssignmentsToTerraform(ctx context.Context, data *TermsAndConditionsResourceModel, assignments []graphmodels.TermsAndConditionsAssignmentable) {
	if len(assignments) == 0 {
		tflog.Debug(ctx, "No assignments to process")
		data.Assignments = types.SetNull(TermsAndConditionsAssignmentType())
		return
	}
	...
	for i, assignment := range assignments {
		...
		case "#microsoft.graph.configurationManagerCollectionAssignmentTarget":
			tflog.Debug(ctx, "Mapping configurationManagerCollectionAssignmentTarget", map[string]any{
				"assignmentIndex": i,
				"assignmentId":    assignment.GetId(),
				"resourceId":      data.ID.ValueString(),
			})
			assignmentObj["type"] = types.StringValue("configurationManagerCollectionAssignmentTarget")

			if configMgrTarget, ok := target.(graphmodels.ConfigurationManagerCollectionAssignmentTargetable); ok {
				collectionId := configMgrTarget.GetCollectionId()
				if collectionId != nil && *collectionId != "" {
					tflog.Debug(ctx, "Setting collection ID for configuration manager collection assignment target", map[string]any{
						"assignmentIndex": i,
						"assignmentId":    assignment.GetId(),
						"collectionId":    *collectionId,
						"resourceId":      data.ID.ValueString(),
					})
					assignmentObj["collection_id"] = convert.GraphToFrameworkString(collectionId)
				} else {
					tflog.Warn(ctx, "Collection ID is nil/empty for configuration manager collection assignment target", map[string]any{
						"assignmentIndex": i,
						"assignmentId":    assignment.GetId(),
						"resourceId":      data.ID.ValueString(),
					})
					assignmentObj["collection_id"] = types.StringNull()
				}
			} else {
				tflog.Error(ctx, "Failed to cast target to ConfigurationManagerCollectionAssignmentTargetable", map[string]any{
					"assignmentIndex": i,
					"assignmentId":    assignment.GetId(),
					"resourceId":      data.ID.ValueString(),
				})
		...
		objValue, diags := types.ObjectValue(TermsAndConditionsAssignmentType().(types.ObjectType).AttrTypes, assignmentObj)
		if !diags.HasError() {
	...
	if len(assignmentValues) > 0 {
		setVal, diags := types.SetValue(TermsAndConditionsAssignmentType(), assignmentValues)
		if diags.HasError() {
