...
resource "random_string" "test_008" {
  length  = 8
  special = false
  upper   = false
}

resource "microsoft365_graph_beta_device_management_windows_update_ring" "test_008" {
  display_name                            = "unit-test-windows-update-ring-008-assignments-minimal-${random_string.test_008.result}"
  description                             = "Minimal Assignments Test"
