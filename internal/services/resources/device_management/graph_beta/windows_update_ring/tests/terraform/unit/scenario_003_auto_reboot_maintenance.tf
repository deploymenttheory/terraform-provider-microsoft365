...
resource "random_string" "test_003" {
  length  = 8
  special = false
  upper   = false
}

resource "microsoft365_graph_beta_device_management_windows_update_ring" "test_003" {
  display_name                            = "unit-test-windows-update-ring-003-auto-reboot-maintenance-${random_string.test_003.result}"
  description                             = "Scenario 3: Auto Install and Reboot at Maintenance Time"
