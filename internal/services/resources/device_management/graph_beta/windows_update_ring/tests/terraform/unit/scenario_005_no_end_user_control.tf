...
resource "random_string" "test_005" {
  length  = 8
  special = false
  upper   = false
}

resource "microsoft365_graph_beta_device_management_windows_update_ring" "test_005" {
  display_name                            = "unit-test-windows-update-ring-005-no-end-user-control-${random_string.test_005.result}"
  description                             = "Scenario 5: Auto Install and Reboot Without End User Control"
