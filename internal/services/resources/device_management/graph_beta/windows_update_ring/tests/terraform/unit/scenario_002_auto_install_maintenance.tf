...
resource "random_string" "test_002" {
  length  = 8
  special = false
  upper   = false
}

resource "microsoft365_graph_beta_device_management_windows_update_ring" "test_002" {
  display_name                            = "unit-test-windows-update-ring-002-auto-install-maintenance-${random_string.test_002.result}"
  description                             = "Scenario 2: Auto Install at Maintenance Time"
