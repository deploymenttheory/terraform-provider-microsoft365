...
func (r WindowsRemediationScriptTestResource) Exists(ctx context.Context, _ any, state *terraform.InstanceState) (*bool, error) {
	...
	url := httpClient.GetBaseURL() + "/deviceManagement/deviceHealthScripts/" + scriptID
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("unexpected error checking Windows remediation script existence: %s (status: %d)", errorInfo.ErrorCode, errorInfo.StatusCode)
	}
