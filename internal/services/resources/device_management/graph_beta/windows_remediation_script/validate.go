...
// 6. Each assignment must have exactly one schedule type defined
	...
	for i, assignment := range assignments {
		...
		if !assignment.FilterId.IsNull() && !assignment.FilterId.IsUnknown() && assignment.FilterId.ValueString() != "" {
			if assignment.FilterType.IsNull() || assignment.FilterType.IsUnknown() || assignment.FilterType.ValueString() == "" {
				diags.AddError(
					"Invalid Assignment Configuration",
					fmt.Sprintf("Assignment at index %d has a filter_id but is missing a filter_type", i),
				)
			} else {
				filterType := assignment.FilterType.ValueString()
				if filterType != "include" && filterType != "exclude" && filterType != "none" {
					diags.AddError(
						"Invalid Assignment Configuration",
						fmt.Sprintf("Assignment at index %d has an invalid filter_type '%s'. Must be one of: 'include', 'exclude', or 'none'", i, filterType),
					)
				}
			}
		}

		// Validate schedule configuration - exactly one schedule type must be defined
		scheduleCount := 0
