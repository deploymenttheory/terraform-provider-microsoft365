...
func constructResource(ctx context.Context, data *WindowsFeatureUpdatePolicyResourceModel, forUpdate bool) (graphmodels.WindowsFeatureUpdateProfileable, error) {
	...
	if !data.RolloutSettings.IsNull() && !data.RolloutSettings.IsUnknown() {
		...
		if diags := data.RolloutSettings.As(ctx, &rolloutModel, basetypes.ObjectAsOptions{}); !diags.HasError() {
			convert.FrameworkToGraphTime(rolloutModel.OfferStartDateTimeInUTC, rolloutSettings.SetOfferStartDateTimeInUTC)
			convert.FrameworkToGraphTime(rolloutModel.OfferEndDateTimeInUTC, rolloutSettings.SetOfferEndDateTimeInUTC)
			convert.FrameworkToGraphInt32(rolloutModel.OfferIntervalInDays, rolloutSettings.SetOfferIntervalInDays)
