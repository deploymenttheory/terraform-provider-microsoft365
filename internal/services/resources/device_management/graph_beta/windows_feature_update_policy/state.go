...
func MapRemoteResourceStateToTerraform(ctx context.Context, data *WindowsFeatureUpdatePolicyResourceModel, remoteResource graphmodels.WindowsFeatureUpdateProfileable) {
	...
	if rolloutSettings := remoteResource.GetRolloutSettings(); rolloutSettings != nil {
		if rolloutSettings.GetOfferStartDateTimeInUTC() != nil ||
			...
			rolloutModel := RolloutSettingsModel{
			...
			rolloutObj, diags := types.ObjectValueFrom(ctx, map[string]attr.Type{
				"offer_start_date_time_in_utc": types.StringType,
				"offer_end_date_time_in_utc":   types.StringType,
				"offer_interval_in_days":       types.Int32Type,
			}, rolloutModel)

			if !diags.HasError() {
				data.RolloutSettings = rolloutObj
			} else {
				tflog.Error(ctx, "Failed to create rollout settings object", map[string]any{
					"errors": diags.Errors(),
				})
				data.RolloutSettings = types.ObjectNull(map[string]attr.Type{
					"offer_start_date_time_in_utc": types.StringType,
					"offer_end_date_time_in_utc":   types.StringType,
					"offer_interval_in_days":       types.Int32Type,
				})
	...
	if len(assignments) == 0 {
		...
		data.Assignments = types.SetNull(WindowsFeatureUpdateProfileAssignmentType())
	} else {
...
// WindowsFeatureUpdateProfileAssignmentType returns the object type for WindowsQualityUpdateProfileAssignmentModel
func WindowsFeatureUpdateProfileAssignmentType() attr.Type {
...
func MapAssignmentsToTerraform(ctx context.Context, data *WindowsFeatureUpdatePolicyResourceModel, assignments []graphmodels.WindowsFeatureUpdateProfileAssignmentable) {
	if len(assignments) == 0 {
	...
	for i, assignment := range assignments {
		...
		objValue, diags := types.ObjectValue(WindowsFeatureUpdateProfileAssignmentType().(types.ObjectType).AttrTypes, assignmentObj)
		if !diags.HasError() {
	...
	if len(assignmentValues) > 0 {
		setVal, diags := types.SetValue(WindowsFeatureUpdateProfileAssignmentType(), assignmentValues)
		if diags.HasError() {
