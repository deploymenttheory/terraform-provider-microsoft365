...
func MapRemoteResourceStateToTerraform(ctx context.Context, data *WindowsQualityUpdateExpeditePolicyResourceModel, remoteResource graphmodels.WindowsQualityUpdateProfileable) {
	...
	if len(assignments) == 0 {
		...
		data.Assignments = types.SetNull(WindowsQualityUpdateProfileAssignmentType())
	} else {
		tflog.Debug(ctx, "Starting assignment stating process", map[string]interface{}{
			"resourceId":      data.ID.ValueString(),
			"assignmentCount": len(assignments),
		})
...
// WindowsQualityUpdateProfileAssignmentType returns the object type for WindowsQualityUpdateProfileAssignmentModel
func WindowsQualityUpdateProfileAssignmentType() attr.Type {
...
func MapAssignmentsToTerraform(ctx context.Context, data *WindowsQualityUpdateExpeditePolicyResourceModel, assignments []graphmodels.WindowsQualityUpdateProfileAssignmentable) {
	if len(assignments) == 0 {
	...
	for i, assignment := range assignments {
		...
		objValue, diags := types.ObjectValue(WindowsQualityUpdateProfileAssignmentType().(types.ObjectType).AttrTypes, assignmentObj)
		if !diags.HasError() {
	...
	if len(assignmentValues) > 0 {
		setVal, diags := types.SetValue(WindowsQualityUpdateProfileAssignmentType(), assignmentValues)
		if diags.HasError() {
