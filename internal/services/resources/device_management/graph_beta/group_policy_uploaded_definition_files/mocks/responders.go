...
var mockState struct {
	sync.Mutex
	groupPolicyUploadedDefinitionFiles map[string]map[string]interface{}
}

func init() {
	mockState.groupPolicyUploadedDefinitionFiles = make(map[string]map[string]interface{})
...
func (m *GroupPolicyUploadedDefinitionFilesMock) RegisterMocks() {
	...
	httpmock.RegisterResponder("GET", "https://graph.microsoft.com/beta/deviceManagement/groupPolicyUploadedDefinitionFiles", func(req *http.Request) (*http.Response, error) {
		...
		list := make([]map[string]interface{}, 0, len(mockState.groupPolicyUploadedDefinitionFiles))
		...
		if v, ok := body["groupPolicyUploadedLanguageFiles"]; ok {
			...
			if languageFiles, ok := v.([]interface{}); ok && len(languageFiles) > 0 {
				...
				for _, lf := range languageFiles {
					if lfMap, ok := lf.(map[string]interface{}); ok {
