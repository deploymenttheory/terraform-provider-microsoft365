...
func LookupDefinitionID(ctx context.Context, client *msgraphbetasdk.GraphServiceClient, displayName, classType, categoryPath string) (string, error) {
...
func LookupDefinitionInfo(ctx context.Context, client *msgraphbetasdk.GraphServiceClient, definitionID string) (displayName, classType, categoryPath string, err error) {
	tflog.Debug(ctx, fmt.Sprintf("[LOOKUP] Looking up definition info for ID='%s'", definitionID))

	definition, err := client.
		DeviceManagement().
		GroupPolicyDefinitions().
