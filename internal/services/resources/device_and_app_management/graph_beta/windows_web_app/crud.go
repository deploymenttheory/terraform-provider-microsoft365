...
func (r *WindowsWebAppResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	windowsWebApp, ok := respBaseResource.(graphmodels.WindowsWebAppable)
	if !ok {
		resp.Diagnostics.AddError(
			"Resource type mismatch",
			fmt.Sprintf("Expected resource of type WindowsWebAppable but got %T", respBaseResource),
		)
		return
	}

	diags := mapResourceToState(ctx, &object, windowsWebApp)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *WindowsWebAppResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, fmt.Sprintf("Deleting Windows Web Clip with ID: %s", appId))
