...
func (m *WindowsManagedMobileAppMock) RegisterMocks() {
	...
	httpmock.RegisterResponder("POST", `=~^https://graph.microsoft.com/beta/deviceAppManagement/windowsManagedAppProtections/[^/]+/apps$`,
		func(req *http.Request) (*http.Response, error) {
			...
			if mobileAppId, ok := appData["mobileAppIdentifier"].(map[string]any); !ok || mobileAppId["windowsAppId"] == nil {
	...
	httpmock.RegisterResponder(constants.TfTfOperationDelete, `=~^https://graph.microsoft.com/beta/deviceAppManagement/windowsManagedAppProtections/[^/]+/apps/[^/]+$`,
		func(req *http.Request) (*http.Response, error) {
			urlParts := strings.Split(req.URL.Path, "/")
			appId := urlParts[len(urlParts)-1]
