...
func (r *MacOSVppAppResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// Construct the base resource from the Terraform model
	...
	// Create the mobile app base resource in Graph API
...
func (r *MacOSVppAppResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	mapResourceToState(ctx, &object, macOSVppApp)
...
func (r *MacOSVppAppResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Update the resource in Graph API
	_, err = r.client.
		DeviceAppManagement().
		MobileApps().
	...
	if !plan.Assignments.Equal(state.Assignments) {
		err = construct.AssignMobileAppAssignments(ctx, r.client, state.ID.ValueString(), plan.Assignments)
		if err != nil {
			errors.HandleGraphError(ctx, err, resp, "Update", r.WritePermissions)
			return
		}
...
func (r *MacOSVppAppResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// Delete the resource from Graph API
	err := r.client.
		DeviceAppManagement().
		MobileApps().
		ByMobileAppId(object.ID.ValueString()).
	...
	if err != nil {
		if errors.IsNotFoundError(err) {
			tflog.Debug(ctx, fmt.Sprintf("Resource with ID %s already deleted or not found", object.ID.ValueString()))
			return
		}
