...
func processSetting(ctx context.Context, setting Setting, settingsRequestPayload *[]graphmodels.DeviceManagementConfigurationSettingable) {
	...
	// Set the @odata.type for the base setting
	baseSetting.SetOdataType(stringPtr("#microsoft.graph.deviceManagementConfigurationSetting"))
	...
	case "choice":
		...
		if setting.SettingInstance.ChoiceSettingValue != nil {
			...
			if len(setting.SettingInstance.ChoiceSettingValue.Children) > 0 {
				...
				// Always set children, even if empty (nil becomes empty array in JSON)
				choiceValue.SetChildren(make([]graphmodels.DeviceManagementConfigurationSettingInstanceable, 0))
			}
