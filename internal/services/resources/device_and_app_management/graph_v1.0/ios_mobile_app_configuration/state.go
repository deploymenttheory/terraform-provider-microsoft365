...
func mapResourceToState(
	...
	tflog.Debug(ctx, "Mapping iOS Mobile App Configuration resource to state")
	...
	if targetedApps == nil || len(targetedApps) == 0 {
	...
	if resource.GetSettings() != nil {
		data.Settings = mapSettingsToState(ctx, resource.GetSettings(), diagnostics)
	}
	...
	// Note: RoleScopeTagIds is not available in the v1.0 API for iOS Mobile App Configuration
	// This field is only available in the beta API
}
...
func mapAssignmentsToState(
	...
	for _, assignment := range assignments {
		...
		if assignment.GetTarget() != nil {
			stateAssignment.Target = mapAssignmentTargetToState(
				ctx,
...
func mapAssignmentTargetToState(
	ctx context.Context,
