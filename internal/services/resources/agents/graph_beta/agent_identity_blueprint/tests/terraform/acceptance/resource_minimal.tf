# Minimal Agent Identity Blueprint configuration for acceptance testing
resource "microsoft365_graph_beta_agents_agent_identity_blueprint" "test_minimal" {
  display_name = "acc-test-agent-identity-blueprint-minimal-${random_string.test_id.result}"
  description  = "This is a test agent identity blueprint with all required fields configured"
  sponsor_user_ids = [
