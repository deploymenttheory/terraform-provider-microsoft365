...
func (r AgentIdentityTestResource) Exists(ctx context.Context, _ any, state *terraform.InstanceState) (*bool, error) {
	...
	agentIdentityID := state.ID

	url := httpClient.GetBaseURL() + "/servicePrincipals/" + agentIdentityID
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("unexpected error checking agent identity existence: %s (status: %d)", errorInfo.ErrorCode, errorInfo.StatusCode)
	}
