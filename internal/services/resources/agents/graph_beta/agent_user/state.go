package graphBetaUsersAgentUser
...
func MapRemoteStateToTerraform(ctx context.Context, data *AgentUserResourceModel, remoteResource graphmodels.Userable, userWithCSA graphmodels.Userable, manager graphmodels.DirectoryObjectable) {
	...
	// Extract identityParentId from additionalData
	if additionalData := remoteResource.GetAdditionalData(); additionalData != nil {
		if identityParentId, ok := additionalData["identityParentId"].(string); ok {
			data.IdentityParentId = types.StringValue(identityParentId)
		}
	}
