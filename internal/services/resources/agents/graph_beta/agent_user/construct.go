...
func constructResource(ctx context.Context, data *AgentUserResourceModel) (graphmodels.Userable, error) {
	...
	// Set @odata.type to specify this is an agentUser
	...
	// Set identityParentId via additionalData (required for agent users)
	additionalData := make(map[string]any)
	if !data.IdentityParentId.IsNull() && !data.IdentityParentId.IsUnknown() {
		additionalData["identityParentId"] = data.IdentityParentId.ValueString()
	}
	...
	// Set all additionalData at once
