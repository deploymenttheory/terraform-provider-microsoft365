...
import (
	...
	graphBetaUsersAgentUser "github.com/deploymenttheory/terraform-provider-microsoft365/internal/services/resources/agents/graph_beta/agent_user"
	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
)

var (
	// Resource type name from the resource package
	resourceType = graphBetaUsersAgentUser.ResourceName

	// testResource is the test resource implementation for users
	testResource = graphBetaUsersAgentUser.UserTestResource{}
)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					check.That(resourceType+".minimal").Key("display_name").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-minimal-[a-z0-9]{8}$`)),
					check.That(resourceType+".minimal").Key("user_principal_name").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-minimal-[a-z0-9]{8}@deploymenttheory\.com$`)),
					check.That(resourceType+".minimal").Key("mail_nickname").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-minimal-[a-z0-9]{8}$`)),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					check.That(resourceType+".maximal").Key("display_name").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-maximal-[a-z0-9]{8}$`)),
					check.That(resourceType+".maximal").Key("user_principal_name").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-maximal-[a-z0-9]{8}@deploymenttheory\.com$`)),
					check.That(resourceType+".maximal").Key("mail_nickname").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-maximal-[a-z0-9]{8}$`)),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					check.That(resourceType+".with_custom_security_attributes").Key("display_name").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-custom-sec-att-[a-z0-9]{8}$`)),
					check.That(resourceType+".with_custom_security_attributes").Key("user_principal_name").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-custom-sec-att-[a-z0-9]{8}@deploymenttheory\.com$`)),
					check.That(resourceType+".with_custom_security_attributes").Key("mail_nickname").MatchesRegex(regexp.MustCompile(`^acc-test-agent-user-custom-sec-att-[a-z0-9]{8}$`)),
