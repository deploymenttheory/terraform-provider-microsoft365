...
func (r AgentCollectionTestResource) Exists(ctx context.Context, _ any, state *terraform.InstanceState) (*bool, error) {
	...
	agentCollectionID := state.ID

	url := httpClient.GetBaseURL() + "/agentRegistry/agentCollections/" + agentCollectionID
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("unexpected error checking agent collection existence: %s (status: %d)", errorInfo.ErrorCode, errorInfo.StatusCode)
	}
