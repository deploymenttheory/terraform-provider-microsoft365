...
func constructResource(ctx context.Context, data *AgentIdentityBlueprintCertificateCredentialResourceModel, existingCredentials []graphmodels.KeyCredentialable) (graphmodels.Applicationable, error) {
	...
	// Set the key (certificate data as bytes)
	convert.FrameworkToGraphBytes(data.Key, newCredential.SetKey)
	...
	// If not replacing, preserve existing credentials
	...
	if !replaceExisting && len(existingCredentials) > 0 {
		...
		keyCredentials = append(keyCredentials, existingCredentials...)
