# Acceptance test: Agent Identity Blueprint Certificate Credential with HEX encoding
# Full dependency chain: random_string -> users -> agent_identity_blueprint -> certificate_credential

resource "random_string" "test_id_hex" {
  length  = 8
  special = false
  upper   = false
}

resource "microsoft365_graph_beta_users_user" "dependency_user_hex_1" {
  display_name        = "acc-test-cert-hex-user1-${random_string.test_id_hex.result}"
  user_principal_name = "acc-test-cert-hex-user1-${random_string.test_id_hex.result}@deploymenttheory.com"
  mail_nickname       = "acc-test-cert-hex-user1-${random_string.test_id_hex.result}"
  account_enabled     = true
  password_profile = {
    password                           = "SecureP@ssw0rd123!"
    force_change_password_next_sign_in = false
  }
}

resource "microsoft365_graph_beta_users_user" "dependency_user_hex_2" {
  display_name        = "acc-test-cert-hex-user2-${random_string.test_id_hex.result}"
  user_principal_name = "acc-test-cert-hex-user2-${random_string.test_id_hex.result}@deploymenttheory.com"
  mail_nickname       = "acc-test-cert-hex-user2-${random_string.test_id_hex.result}"
  account_enabled     = true
  password_profile = {
    password                           = "SecureP@ssw0rd123!"
    force_change_password_next_sign_in = false
  }
}

resource "microsoft365_graph_beta_agents_agent_identity_blueprint" "test_blueprint_hex" {
  display_name = "acc-test-blueprint-cert-hex-${random_string.test_id_hex.result}"
  description  = "Agent identity blueprint for HEX certificate credential acceptance test"
  sponsor_user_ids = [
    microsoft365_graph_beta_users_user.dependency_user_hex_1.id,
    microsoft365_graph_beta_users_user.dependency_user_hex_2.id,
  ]
  owner_user_ids = [
    microsoft365_graph_beta_users_user.dependency_user_hex_1.id,
    microsoft365_graph_beta_users_user.dependency_user_hex_2.id,
  ]
  hard_delete = true
}

resource "microsoft365_graph_beta_agents_agent_identity_blueprint_certificate_credential" "test_hex" {
  blueprint_id = microsoft365_graph_beta_agents_agent_identity_blueprint.test_blueprint_hex.id
  display_name = "acc-test-certificate-hex-${random_string.test_id_hex.result}"

  # Pre-generated hex-encoded certificate (generated with generate_hex_cert.sh)
  key      = ""
  encoding = "hex"
  type     = "AsymmetricX509Cert"
  usage    = "Verify"
}
