...
func (r ConditionalAccessTermsOfUseTestResource) Exists(ctx context.Context, _ any, state *terraform.InstanceState) (*bool, error) {
	...
	url := httpClient.GetBaseURL() + "/agreements/" + agreementID
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("unexpected error checking agreement existence: %s (status: %d)", errorInfo.ErrorCode, errorInfo.StatusCode)
	}

	var agreement map[string]any
	if err := json.NewDecoder(httpResp.Body).Decode(&agreement); err != nil {
