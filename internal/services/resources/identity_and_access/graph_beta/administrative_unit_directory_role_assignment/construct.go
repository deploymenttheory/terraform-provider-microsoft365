...
func constructResource(ctx context.Context, data *AdministrativeUnitDirectoryRoleAssignmentResourceModel) (graphmodels.ScopedRoleMembershipable, error) {
	tflog.Debug(ctx, fmt.Sprintf("Constructing %s resource from model", ResourceName))

	requestBody := graphmodels.NewScopedRoleMembership()

	convert.FrameworkToGraphString(data.DirectoryRoleID, requestBody.SetRoleId)
