...
resource "microsoft365_graph_beta_groups_group" "cal004_exclude" {
  display_name     = "EID_UA_CAL004_EXCLUDE"
  mail_nickname    = "eid-ua-cal004-exclude"
  mail_enabled     = false
  security_enabled = true
  description      = "uexcludeion group for CA policy CAL004_EXCLUDE"
}
...
resource "microsoft365_graph_beta_identity_and_access_conditional_access_policy" "cal004_block_admin_untrusted_locations" {
  display_name = "acc-test-cal004-all: Block access for Admins from non-trusted locations when Browser and Modern Auth Clients ${random_string.suffix.result}"
