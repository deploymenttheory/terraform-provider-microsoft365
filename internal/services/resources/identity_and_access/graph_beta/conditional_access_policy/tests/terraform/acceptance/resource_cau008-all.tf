...
resource "microsoft365_graph_beta_groups_group" "cau008_exclude" {
  display_name     = "EID_UA_CAU008_EXCLUDE"
  mail_nickname    = "eid-ua-cau008-exclude"
  mail_enabled     = false
  security_enabled = true
  description      = "uexcludeion group for CA policy CAU008_EXCLUDE"
}
...
resource "microsoft365_graph_beta_identity_and_access_conditional_access_policy" "cau008_admin_phishing_resistant_mfa" {
  display_name = "acc-test-cau008-all: Grant Require Phishing Resistant MFA for Admins when Browser and Modern Auth Clients ${random_string.suffix.result}"
