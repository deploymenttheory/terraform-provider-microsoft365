...
resource "microsoft365_graph_beta_groups_group" "cad012_exclude" {
  display_name     = "EID_UA_CAD012_EXCLUDE"
  mail_nickname    = "eid-ua-cad012-exclude"
  mail_enabled     = false
  security_enabled = true
  description      = "uexcludeion group for CA policy CAD012_EXCLUDE"
}
...
resource "microsoft365_graph_beta_identity_and_access_conditional_access_policy" "cad012_admin_compliant_access" {
  display_name = "acc-test-cad012-all: Grant access for Admin users when Browser and Modern Auth Clients and Compliant ${random_string.suffix.result}"
