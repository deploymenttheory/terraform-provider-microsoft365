...
resource "microsoft365_graph_beta_groups_group" "cal002_exclude" {
  display_name     = "EID_UA_CAL002_EXCLUDE"
  mail_nickname    = "eid-ua-cal002-exclude"
  mail_enabled     = false
  security_enabled = true
  description      = "uexcludeion group for CA policy CAL002_EXCLUDE"
}
...
resource "microsoft365_graph_beta_identity_and_access_conditional_access_policy" "cal002_mfa_registration_trusted_locations" {
  display_name = "acc-test-cal002-rsi: Require MFA registration from trusted locations only for All users when Browser and Modern Auth Clients ${random_string.suffix.result}"
