...
func TestAccResourceNetworkThreatIntelligencePolicy_01_Lifecycle(t *testing.T) {
	testResource := target.NetworkThreatIntelligencePolicyTestResource{}
	...
	sameID := func(s *terraform.State) error {
		...
		t.Logf("Threat intelligence policy ID: %s", id)
		if savedID == "" {
			savedID = id
		} else if id != savedID {
