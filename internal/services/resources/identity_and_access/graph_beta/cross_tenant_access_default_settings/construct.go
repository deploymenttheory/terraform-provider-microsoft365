...
// constructResource converts the Terraform resource model to a CrossTenantAccessPolicyConfigurationDefault SDK request body.
...
func constructResource(ctx context.Context, data *CrossTenantAccessDefaultSettingsResourceModel) (graphmodels.CrossTenantAccessPolicyConfigurationDefaultable, error) {
	...
	if !data.InvitationRedemptionIdentityProviderConfiguration.IsNull() && !data.InvitationRedemptionIdentityProviderConfiguration.IsUnknown() {
		invitationConfig, err := constructInvitationRedemptionConfig(ctx, data.InvitationRedemptionIdentityProviderConfiguration)
		if err != nil {
			return nil, fmt.Errorf("failed to construct invitation_redemption_identity_provider_configuration: %w", err)
		}
		requestBody.SetInvitationRedemptionIdentityProviderConfiguration(invitationConfig)
	}
...
// constructB2BSetting constructs a CrossTenantAccessPolicyB2BSetting from a types.Object
...
// constructTargetConfiguration constructs a CrossTenantAccessPolicyTargetConfiguration
...
// constructInboundTrust constructs a CrossTenantAccessPolicyInboundTrust
...
// constructInvitationRedemptionConfig constructs a DefaultInvitationRedemptionIdentityProviderConfiguration
func constructInvitationRedemptionConfig(_ context.Context, obj types.Object) (graphmodels.DefaultInvitationRedemptionIdentityProviderConfigurationable, error) {
	if obj.IsNull() || obj.IsUnknown() {
		return nil, nil
	}

	config := graphmodels.NewDefaultInvitationRedemptionIdentityProviderConfiguration()

	attrs := obj.Attributes()

	// PrimaryIdentityProviderPrecedenceOrder
	// Note: must use a list to ensure ordering is preserved.
	if precedenceList, ok := attrs["primary_identity_provider_precedence_order"].(types.List); ok && !precedenceList.IsNull() {
		var precedenceOrder []graphmodels.B2bIdentityProvidersType
		for _, elem := range precedenceList.Elements() {
			if strVal, ok := elem.(types.String); ok && !strVal.IsNull() {
				if parsed, err := graphmodels.ParseB2bIdentityProvidersType(strVal.ValueString()); err == nil && parsed != nil {
					precedenceOrder = append(precedenceOrder, *parsed.(*graphmodels.B2bIdentityProvidersType))
				}
			}
		}
		if len(precedenceOrder) > 0 {
			config.SetPrimaryIdentityProviderPrecedenceOrder(precedenceOrder)
		}
	}

	// FallbackIdentityProvider
	if fallback, ok := attrs["fallback_identity_provider"].(types.String); ok && !fallback.IsNull() {
		if parsed, err := graphmodels.ParseB2bIdentityProvidersType(fallback.ValueString()); err == nil && parsed != nil {
			val := parsed.(*graphmodels.B2bIdentityProvidersType)
			config.SetFallbackIdentityProvider(val)
		}
	}
...
// constructTenantRestrictions constructs a CrossTenantAccessPolicyTenantRestrictions
...
// constructAutomaticUserConsentSettings constructs an InboundOutboundPolicyConfiguration
