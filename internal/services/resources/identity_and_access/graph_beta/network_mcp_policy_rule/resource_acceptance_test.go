...
func TestAccResourceNetworkMCPPolicyRule_01_Lifecycle(t *testing.T) {
	testResource := target.NetworkMCPPolicyRuleTestResource{}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: config("resource_no_conditions.tf"), Check: resource.ComposeTestCheckFunc(sameID, check.That(resourceType+".test").Key("enabled").HasValue("true"), check.That(resourceType+".test").Key("matching_conditions").DoesNotExist())},
		}})
}
