...
var conditionKeys = map[string]string{
	"server_urls": "serverUrls", "protocol_versions": "protocolVersions",
	"insecure_connection": "insecureConnection", "missing_prm": "missingPrm",
	"tool_matching": "toolMatching", "resource_matching": "resourceMatching", "prompt_matching": "promptMatching",
}

func matchSchema() map[string]schema.Attribute {
	return map[string]schema.Attribute{
		"values":     schema.ListAttribute{MarkdownDescription: "Values in API order. Empty arrays are preserved and are distinct from an omitted condition. Pattern evaluation is performed by the service; regex or wildcard behavior is not guaranteed by this resource.", Required: true, ElementType: types.StringType},
		"match_type": schema.StringAttribute{MarkdownDescription: "Match operator: exactMatch, contains, notExactMatch, or notContains. Must be explicitly configured.", Required: true, Validators: []validator.String{stringvalidator.OneOf("exactMatch", "contains", "notExactMatch", "notContains")}},
	}
}
func primitiveSchema() map[string]schema.Attribute {
	return map[string]schema.Attribute{
		"names":   schema.SingleNestedAttribute{MarkdownDescription: "Primitive name matching.", Optional: true, Attributes: matchSchema()},
		"methods": schema.StringAttribute{MarkdownDescription: "API method flags, for example call or list,call for tools, read for resources, or get for prompts. Values are sent unchanged; unsupported combinations return API diagnostics.", Optional: true, Validators: []validator.String{stringvalidator.LengthAtLeast(1)}},
	}
}
func conditionSchema() map[string]schema.Attribute {
	...
	for name := range conditionKeys {
		...
		case "insecure_connection", "missing_prm":
			result[name] = schema.StringAttribute{MarkdownDescription: "Whether this property is required or excluded by the condition. Omission removes this constraint.", Optional: true, Validators: []validator.String{stringvalidator.OneOf("required", "excluded")}}
		case "tool_matching", "resource_matching", "prompt_matching":
			result[name] = schema.SingleNestedAttribute{MarkdownDescription: "Primitive conditions applied to the rule. Multiple primitive types may be stored by the API; their combined traffic evaluation is not verified.", Optional: true, Attributes: primitiveSchema()}
		default:
			result[name] = schema.SingleNestedAttribute{MarkdownDescription: "String matching condition. Values are preserved without URL or case normalization.", Optional: true, Attributes: matchSchema()}
		}
	}
	return result
}
...
func ruleConditions(ctx context.Context, data *NetworkMCPPolicyRuleResourceModel) (*conditionPayload, error) {
...
func (b *conditionPayload) Serialize(w s.SerializationWriter) error {
	for key, value := range *b {
		...
		default:
			return fmt.Errorf("unsupported MCP condition value %T", value)
		}
		if err != nil {
...
func conditionsToState(ctx context.Context, raw json.RawMessage) (types.Object, error) {
	...
	if err := json.Unmarshal(raw, &envelope); err != nil {
		return types.ObjectNull(conditionAttributeTypes()), fmt.Errorf("%w: %v", errInvalidResponse, err)
	}
	for key, value := range envelope {
		if key != "destinations" && nonNull(value) {
			return types.ObjectNull(conditionAttributeTypes()), fmt.Errorf("%w: unsupported matchingConditions.%s", errInvalidResponse, key)
		}
	}
...
func objectFromJSON(ctx context.Context, raw json.RawMessage, typ types.ObjectType, keys map[string]string) (types.Object, error) {
	var values map[string]json.RawMessage
	if err := json.Unmarshal(raw, &values); err != nil {
		return types.ObjectNull(typ.AttrTypes), fmt.Errorf("%w: %v", errInvalidResponse, err)
	}
	if _, matching := typ.AttrTypes["match_type"]; matching {
		if !nonNull(values["values"]) || !nonNull(values["matchType"]) {
			return types.ObjectNull(typ.AttrTypes), fmt.Errorf("%w: match condition requires values and matchType", errInvalidResponse)
		}
	}
	...
	for tf, t := range typ.AttrTypes {
		...
		case basetypes.StringType:
			...
			if nonNull(value) {
				if err := json.Unmarshal(value, &v); err != nil {
					return types.ObjectNull(typ.AttrTypes), fmt.Errorf("%w: %s: %v", errInvalidResponse, api, err)
				}
			}
			if v != nil {
				allowed := []string(nil)
				...
				if allowed != nil && !slices.Contains(allowed, *v) {
					return types.ObjectNull(typ.AttrTypes), fmt.Errorf("%w: unsupported %s %q", errInvalidResponse, api, *v)
				}
			}
		...
		case types.ListType:
			...
			for _, item := range items {
				if item == nil {
					return types.ObjectNull(typ.AttrTypes), fmt.Errorf("%w: null list element", errInvalidResponse)
				}
	...
	for key, value := range values {
		if !known[key] && nonNull(value) {
			return types.ObjectNull(typ.AttrTypes), fmt.Errorf("%w: unsupported non-null MCP condition %s", errInvalidResponse, key)
		}
	}
