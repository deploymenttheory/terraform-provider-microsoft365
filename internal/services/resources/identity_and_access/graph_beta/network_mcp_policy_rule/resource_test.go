...
func TestUnitResourceNetworkMCPPolicyRule_01_Lifecycle(t *testing.T) {
	...
	resource.UnitTest(t, resource.TestCase{ProtoV6ProviderFactories: mocks.TestUnitTestProtoV6ProviderFactories, Steps: []resource.TestStep{
		...
		{Config: testConfigHelper("resource_no_conditions.tf"), Check: resource.ComposeTestCheckFunc(sameID, check.That(resourceType+".test").Key("enabled").HasValue("true"), check.That(resourceType+".test").Key("matching_conditions").DoesNotExist())},
	}})
	m.Lock()
	defer m.Unlock()
...
func TestUnitResourceNetworkMCPPolicyRule_04_PriorityAndConditionValidation(t *testing.T) {
	for _, tc := range []struct{ name, old, new, pattern string }{
		...
		{"invalid-match", "exactMatch", "regex", `value must be one of`},
	} {
...
resource "microsoft365_graph_beta_identity_and_access_network_mcp_policy_rule" "conflict" {
 ...
 depends_on = [microsoft365_graph_beta_identity_and_access_network_mcp_policy_rule.test]
	...
	resource.UnitTest(t, resource.TestCase{ProtoV6ProviderFactories: mocks.TestUnitTestProtoV6ProviderFactories, Steps: []resource.TestStep{{Config: config, ExpectError: regexp.MustCompile("priority already exists")}}})
}
