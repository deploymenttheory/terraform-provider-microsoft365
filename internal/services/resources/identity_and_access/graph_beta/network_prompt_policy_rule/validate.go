...
func (r *NetworkPromptPolicyRuleResource) ValidateConfig(
	ctx context.Context,
	...
	if resp.Diagnostics.HasError() || data.ConversationSchemes.IsNull() ||
		data.ConversationSchemes.IsUnknown() {
		return
	}
	for i, element := range data.ConversationSchemes.Elements() {
		...
		if element.IsNull() {
			resp.Diagnostics.AddAttributeError(
				p,
				"Invalid conversation scheme",
				"A scheme cannot be null.",
			)
			continue
		}
		...
		case schemeTypeCustom:
			...
			if !scheme.URL.IsNull() && !scheme.URL.IsUnknown() {
				...
				if err != nil || u.Host == "" || u.Path == "" ||
					...
					resp.Diagnostics.AddAttributeError(
						p.AtName("url"),
						"Invalid custom URL",
						"Use an absolute HTTP or HTTPS URL with a path, such as /chat or /.",
					)
				}
			}
			if !scheme.SchemeName.IsNull() {
				resp.Diagnostics.AddAttributeError(
					...
					"Custom schemes cannot specify scheme_name.",
				)
			}
		case schemeTypePredefined:
			...
			if !scheme.URL.IsNull() || !scheme.JSONPath.IsNull() {
				resp.Diagnostics.AddAttributeError(
					...
					"Predefined schemes cannot specify url or json_path.",
				)
			}
		}
	}
