...
func (m *PromptPolicyRuleMock) RegisterErrorMocks() {
	httpmock.RegisterResponder("POST", `=~^https://graph\.microsoft\.com/beta/networkaccess/promptPolicies/[^/]+/policyRules$`, httpmock.NewStringResponder(400, `{"error":{"code":"BadRequest","message":"Synthetic prompt rule validation error"}}`))
}
