...
func MapRemoteStateToTerraform(ctx context.Context, data *NetworkPromptPolicyRuleResourceModel, remote *promptPolicyRuleResponse) error {
	if remote == nil || remote.id == nil || *remote.id == "" || remote.name == nil || remote.action == nil || remote.priority == nil || remote.status == nil || remote.promptLogging == nil || remote.odataType == nil || remote.conditions == nil || remote.conditions.scanResult == nil || !remote.conditions.schemesPresent {
	...
	if *remote.priority < 100 || *remote.priority > math.MaxInt32 {
		return fmt.Errorf("%w: priority %d outside supported range", errInvalidResponse, *remote.priority)
	}
	if *remote.status != "enabled" && *remote.status != "disabled" {
		return fmt.Errorf("%w: unsupported status %q; enabled cannot be determined", errInvalidResponse, *remote.status)
	}
	...
	for i, scheme := range remote.conditions.schemes {
		...
		fields := map[string]attr.Value{"type": types.StringNull(), "url": types.StringNull(), "json_path": types.StringNull(), "scheme_name": types.StringNull()}
		...
		case "microsoft.graph.networkaccess.customConversationScheme":
			...
			if *scheme.jsonPath == "" && i < len(prior) && prior[i].Type.ValueString() == schemeTypeCustom && prior[i].URL.ValueString() == *scheme.url && prior[i].JSONPath.IsNull() {
		...
		default:
			return fmt.Errorf("%w: unsupported scheme type %q", errInvalidResponse, *scheme.odataType)
		}
		schemes = append(schemes, types.ObjectValueMust(conversationSchemeObjectType().AttrTypes, fields))
	}
...
func conversationSchemeObjectType() types.ObjectType {
	return types.ObjectType{AttrTypes: map[string]attr.Type{"type": types.StringType, "url": types.StringType, "json_path": types.StringType, "scheme_name": types.StringType}}
}
