...
// validateRequest validates the partner group sync settings request
func validateRequest(ctx context.Context, client *msgraphbetasdk.GraphServiceClient, data *CrossTenantAccessPartnerGroupSyncSettingsResourceModel) error {
	tflog.Debug(ctx, "Starting partner group sync settings request validation")
	...
	tflog.Debug(ctx, "Partner group sync settings request validation completed successfully")
	return nil
}
