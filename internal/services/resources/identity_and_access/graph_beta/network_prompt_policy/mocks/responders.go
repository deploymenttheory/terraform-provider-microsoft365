...
func (m *PromptPolicyMock) RegisterMocks() {
	...
	for _, method := range []string{"POST", "GET", "PATCH", "DELETE"} {
		httpmock.RegisterResponder(method, `=~^https://graph\.microsoft\.com/beta/networkaccess/promptPolicies(?:/.*)?$`, m.respond)
	}
}
func (m *PromptPolicyMock) RegisterErrorMocks() {
	httpmock.RegisterResponder("POST", BaseURL, func(*http.Request) (*http.Response, error) { return apiError(400, "Invalid prompt policy") })
}
...
func (m *PromptPolicyMock) respond(req *http.Request) (*http.Response, error) {
	...
	if req.Method == "POST" || req.Method == "PATCH" {
		if body["@odata.type"] != "#microsoft.graph.networkaccess.promptRule" {
			return apiError(400, "Explicit prompt rule discriminator required")
		}
		...
		if v, ok := body["promptLogging"]; ok && v != "never" && v != "always" && v != "onBlock" {
			return apiError(400, "Invalid prompt logging")
		}
		...
		if conditions, ok := body["matchingConditions"].(map[string]any); ok {
			...
			for _, value := range schemes {
				...
				default:
					return apiError(400, "Explicit scheme discriminator required")
				}
			}
	...
	if len(parts) == 2 {
		...
		if req.Method == "POST" {
			...
			id := uuid.NewString()
			rule := map[string]any{"id": id, "description": nil, "action": "allow", "promptLogging": "never"}
			for k, v := range body {
				rule[k] = v
			}
			...
			rules[id] = rule
			policy["lastModifiedDateTime"] = time.Now().UTC().Format(time.RFC3339Nano)
			response, err := httpmock.NewJsonResponse(200, rule)
