...
func (r *SubscriptionResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	var object SubscriptionResourceModel
...
func (r *SubscriptionResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	patchBody, err := constructPatch(ctx, &plan)
	if err != nil {
		resp.Diagnostics.AddError(
			"Error constructing patch body",
			fmt.Sprintf("Could not build update for %s: %s", ResourceName, err.Error()),
		)
		return
	}

	_, err = r.client.Subscriptions().
		BySubscriptionId(state.ID.ValueString()).
...
func (r *SubscriptionResource) Delete(
	ctx context.Context,
	...
	err := r.client.Subscriptions().BySubscriptionId(object.ID.ValueString()).Delete(ctx, nil)
	if err != nil {
		errors.HandleKiotaGraphError(
			ctx,
			err,
