...
resource "microsoft365_graph_beta_windows_updates_autopatch_deployment_audience" "test_audience_02" {
}

resource "microsoft365_graph_beta_windows_updates_update_policy" "test_policy_02" {
  audience_id        = microsoft365_graph_beta_windows_updates_autopatch_deployment_audience.test_audience_02.id
...
resource "microsoft365_graph_beta_windows_updates_autopatch_content_approval" "test" {
  ...
  deployment_settings = {
    schedule = {
      ...
      gradual_rollout = {
        end_date_time = "2026-06-30T00:00:00Z"
      }
    }
  }
}
