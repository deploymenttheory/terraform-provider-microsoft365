...
verifyLoop:
	...
	// Clean up implicit deployments created by this content approval.
	// The Graph API creates a deployment bound to the update policy's audience when
	// a content approval is created. These deployments are not automatically removed
	// when the compliance change is deleted, which prevents the audience from being
	// deleted later. This is best-effort: failures are logged but do not block the
	// compliance change from being removed from Terraform state.
	tflog.Debug(ctx, fmt.Sprintf("Cleaning up implicit deployments for update policy %s", object.UpdatePolicyId.ValueString()))

	policy, policyErr := r.client.
		Admin().
		Windows().
		Updates().
		UpdatePolicies().
	...
	if policyErr != nil {
		tflog.Warn(ctx, fmt.Sprintf("Could not retrieve update policy for deployment cleanup: %s", policyErr.Error()))
	} else if policy != nil && policy.GetAudience() != nil && policy.GetAudience().GetId() != nil {
		audienceID := *policy.GetAudience().GetId()
		tflog.Debug(ctx, fmt.Sprintf("Found audience ID %s, listing deployments for cleanup", audienceID))

		deploymentsResult, deployErr := r.client.
			Admin().
			Windows().
			Updates().
			Deployments().
		...
		if deployErr != nil {
			tflog.Warn(ctx, fmt.Sprintf("Could not list deployments for cleanup: %s", deployErr.Error()))
		} else if deploymentsResult != nil {
			for _, deployment := range deploymentsResult.GetValue() {
				if deployment == nil || deployment.GetId() == nil {
					continue
				}
				if deployment.GetAudience() == nil || deployment.GetAudience().GetId() == nil {
					continue
				}
				if *deployment.GetAudience().GetId() != audienceID {
					continue
				}
				deploymentID := *deployment.GetId()
				tflog.Debug(ctx, fmt.Sprintf("Deleting implicit deployment %s (audience %s)", deploymentID, audienceID))
				delErr := r.client.
					Admin().
					Windows().
					Updates().
					...
					Delete(ctx, nil)
				if delErr != nil {
					tflog.Warn(ctx, fmt.Sprintf("Could not delete implicit deployment %s: %s", deploymentID, delErr.Error()))
				} else {
					tflog.Debug(ctx, fmt.Sprintf("Successfully deleted implicit deployment %s", deploymentID))
				}
			}
		}
	}
