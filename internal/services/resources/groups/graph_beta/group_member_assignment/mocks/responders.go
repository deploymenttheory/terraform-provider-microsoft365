...
var mockState struct {
	...
	groupMembers map[string]map[string]interface{} // groupId -> memberId -> memberData
}

func init() {
	// Initialize mockState
	mockState.groups = make(map[string]map[string]interface{})
	mockState.groupMembers = make(map[string]map[string]interface{})
...
func (m *GroupMemberAssignmentMock) RegisterMocks() {
	...
	baseGroupData := map[string]interface{}{
		"id":              baseGroupId,
	...
	mockState.groupMembers[baseGroupId] = make(map[string]interface{})
	mockState.Unlock()
	...
	httpmock.RegisterResponder("GET", `=~^https://graph.microsoft.com/beta/groups/[^/]+/members$`,
		func(req *http.Request) (*http.Response, error) {
			...
			members := make([]map[string]interface{}, 0, len(membersMap))
			for _, memberData := range membersMap {
				if md, ok := memberData.(map[string]interface{}); ok {
	...
	httpmock.RegisterResponder("POST", `=~^https://graph.microsoft.com/beta/groups/[^/]+/members/\$ref$`,
		func(req *http.Request) (*http.Response, error) {
			...
			if mockState.groupMembers[groupId] == nil {
				mockState.groupMembers[groupId] = make(map[string]interface{})
			}
...
func registerSpecificGroupMocks() {
	...
	mockState.groupMembers[minimalGroupId] = make(map[string]interface{})
	mockState.Unlock()
	...
	mockState.groupMembers[maximalGroupId] = make(map[string]interface{})
	mockState.Unlock()
	...
	maximalMemberData := map[string]interface{}{
		"id":              maximalMemberId,
		"@odata.type":     "#microsoft.graph.group",
		"displayName":     "Maximal Group",
		"description":     "A nested group for testing",
