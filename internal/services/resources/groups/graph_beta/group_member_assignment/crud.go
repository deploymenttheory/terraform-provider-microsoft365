...
func (r *GroupMemberAssignmentResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if plan.GroupID.ValueString() != state.GroupID.ValueString() ||
		...
		if err != nil {
			errors.HandleKiotaGraphError(ctx, err, resp, "Update (Remove old member)", r.WritePermissions)
			return
		}
		...
		if err != nil {
			errors.HandleKiotaGraphError(ctx, err, resp, "Update (Add new member)", r.WritePermissions)
			return
		}
