...
func (r *GroupAppRoleAssignmentResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if plan.TargetGroupID.ValueString() != state.TargetGroupID.ValueString() ||
		...
		if err != nil {
			errors.HandleKiotaGraphError(ctx, err, resp, "Update (Remove old assignment)", r.WritePermissions)
			return
		}
		...
		if err != nil {
			errors.HandleKiotaGraphError(ctx, err, resp, "Update (Add new assignment)", r.WritePermissions)
			return
		}
