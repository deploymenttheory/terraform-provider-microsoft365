...
func (r RoleDefinitionTestResource) Exists(ctx context.Context, _ any, state *terraform.InstanceState) (*bool, error) {
	...
	url := httpClient.GetBaseURL() + "/roleManagement/cloudPC/roleDefinitions/" + definitionID
	...
	if httpResp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("unexpected error checking definition existence: %s (status: %d)", errorInfo.ErrorCode, httpResp.StatusCode)
	}
