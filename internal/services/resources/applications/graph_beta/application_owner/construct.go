...
// constructResource constructs a reference object for adding an owner to an application
func constructResource(ctx context.Context, data *ApplicationOwnerResourceModel, client *msgraphbetasdk.GraphServiceClient) (graphmodels.ReferenceCreateable, error) {
	tflog.Debug(ctx, fmt.Sprintf("Constructing %s resource from model", ResourceName))

	if err := ValidateApplicationOwner(ctx, client, *data, false); err != nil {
		return nil, err
	}
