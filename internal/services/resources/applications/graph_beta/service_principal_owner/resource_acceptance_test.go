...
func TestAccResourceServicePrincipalOwner_01_OwnerTypeUser(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				PreConfig: func() {
					testlog.StepAction(resourceType, "Step 1: Creating service principal owner assignment with User owner type")
				},
				Config: loadAccTestTerraform("resource_01_owner_type_user.tf"),
				Check: resource.ComposeTestCheckFunc(
					func(s *terraform.State) error {
						testlog.WaitForConsistency("service principal owner", 30*time.Second)
						time.Sleep(30 * time.Second)
						return nil
					},
					check.That(resourceType+".test_user").Key("id").MatchesRegex(regexp.MustCompile(`^[0-9a-fA-F-]+/[0-9a-fA-F-]+$`)),
					check.That(resourceType+".test_user").Key("service_principal_id").MatchesRegex(regexp.MustCompile(`^[0-9a-fA-F-]+$`)),
...
func TestAccResourceServicePrincipalOwner_02_OwnerTypeServicePrincipal(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { mocks.TestAccPreCheck(t) },
		ProtoV6ProviderFactories: mocks.TestAccProtoV6ProviderFactories,
		CheckDestroy: destroy.CheckDestroyedAllFunc(
		...
		Steps: []resource.TestStep{
			{
				PreConfig: func() {
					testlog.StepAction(resourceType, "Step 1: Creating service principal owner assignment with ServicePrincipal owner type")
				},
				...
				Check: resource.ComposeTestCheckFunc(
					...
					check.That(resourceType+".test_service_principal").Key("service_principal_id").MatchesRegex(regexp.MustCompile(`^[0-9a-fA-F-]+$`)),
