...
// constructResource constructs a reference object for adding an owner to a service principal
func constructResource(ctx context.Context, data *ServicePrincipalOwnerResourceModel, client *msgraphbetasdk.GraphServiceClient) (graphmodels.ReferenceCreateable, error) {
	tflog.Debug(ctx, fmt.Sprintf("Constructing %s resource from model", ResourceName))

	if err := ValidateServicePrincipalOwner(ctx, client, *data, false); err != nil {
		return nil, err
	}
